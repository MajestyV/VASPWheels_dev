...
if __name__=='__main__':
    ...
    # 自定义颜色条
    cmap = cm.RdBu  # 获取Spectral色条，Spectral_r即为反色
    newcolors = cmap(np.linspace(0, 1, 256))  # 分片操作
    print(newcolors[128])
    newcmap = ListedColormap(newcolors[128:])  # 只取128以后的颜色列表，后面的舍去
    ...
    vw.Visualization_MultiPlot.VisualizeScatter_Fatband(len(layer_list), data, (1, 5),
                                         ...
                                         colormap=newcmap,color_background=np.array([0.9657055,0.96724337,0.9680892]),colormap_norm=(0, 1.0))
    ...
    # 可视化并保存scalebar
    vw.CustomizingColormap.ShowColorbar(newcmap,(0,1))
