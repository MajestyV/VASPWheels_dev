...
GetK = GetKpath.Kpath()
Visual = Visualization.plot()

Visual.GlobalSetting()  # 基础全局画图设定
...
def Ebands(EIGENVAL,Kpath,fermi_energy,lattice=('HEX', [3.16, 3.16, 12.9, 90, 90, 120], 'primitive')):
    ...
    # 确定K点路径（X轴）
    k, knodes = GetK.ProjectKpath(Kpath, npoints, LatticeCorrection='True', Lattice=lattice)  # 生成投影到一维的K点路径
...
if __name__=='__main__':
    ...
    for n in data_list:
        ...
        DOS_data = GetD.GetData(DOSCAR)
        dos_x.append(DOS_data['DOS'])
        dos_energy = DOS_data['energy']
        dos_y.append(GetD.ShiftFermiEnergy(dos_energy, E_fermi))
        ###########################################
        ...
        bands_plot.set_title(r'$\varepsilon$ = '+Efield[bands_index],size=22)
    ...
    for i in range(3):
        DOS_fig.plot(dos_x[i],dos_y[i],color=color[i],label=r'$\varepsilon$ = '+Efield[i])
