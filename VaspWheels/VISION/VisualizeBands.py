...
def VisualizeElectronicBands(Kpath_projected,Bands,Knodes_projected,**kwargs):
    ...
    color = kwargs['color'] if 'color' in kwargs else vw.colors.crayons['Navy Blue']  # 能带曲线颜色
    color_split = kwargs['color_split'] if 'color_split' in kwargs else vw.colors.crayons['Gray']  # 分割线颜色
    ...
    for i in range(num_bands):
        vw.Visualization.Visualize(Kpath_projected,Bands[i],color=color)
    ...
    vw.Visualization.CustomSetting(xlim=xlim, ylim=ylim, title=title, ylabel=ylabel)  # 对能带图进行个性化设置
